...
def run_pipeline():
    ...
    for file_path in pdf_files:
        ...
        doc_dict = process_pdf_data(raw_data)
        
        # Bước 2: Kiểm tra chất lượng (run_semantic_checks). 
        # Nếu đạt (True) thì thêm vào list final_kb
        if run_semantic_checks(doc_dict):
            final_kb.append(UnifiedDocument(**doc_dict).model_dump())
    ...
    for file_path in video_files:
        ...
        # Làm tương tự như phần PDF
        doc_dict = process_video_data(raw_data)
        if run_semantic_checks(doc_dict):
            final_kb.append(UnifiedDocument(**doc_dict).model_dump())
